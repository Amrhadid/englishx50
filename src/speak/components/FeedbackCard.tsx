...
export default function FeedbackCard({ feedback, compact = false }: { feedback: SpeakFeedback; compact?: boolean }) {
  return (
    <section
      className={`spk-feedback ${compact ? 'is-compact' : ''}`}
      aria-label={T.feedbackTitle}
    >
      <header className="spk-feedback-header">
        <EmmaAvatar size={38} />
        <div><strong>{T.feedbackTitle}</strong><span>بعد إجابتك</span></div>
        <span className="spk-feedback-check"><CheckIcon /></span>
      </header>
      ...
      <div className="spk-feedback-ready"><span aria-hidden="true">✓</span> جاهز للدور الجاي</div>
    </section>
  )
}
