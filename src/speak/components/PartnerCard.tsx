...
export default function PartnerCard({ level, scenario, phase, prompt, onReplay, canReplay }: Props) {
  const speaking = phase === 'speaking'
  const scenarioLabel = SCENARIOS.find((item) => item.id === scenario)?.label
  return (
    <section className={`spk-stage ${speaking ? 'is-speaking' : ''}`}
    ...
    >
      <div className="spk-session-status">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <strong className="spk-en">Emma</strong>
            <span className="spk-online-dot" aria-hidden="true" />
            <span>{T.statusPill}</span>
          </div>
          <p>{scenarioLabel} · {T.levelPrefix} {levelLabel(level)}</p>
        </div>
      </div>
      ...
      <div className="spk-stage-copy">
        <div>
          <p className="spk-stage-kicker">{speaking ? 'Emma بترد عليك…' : 'دورك في المحادثة'}</p>
          <p className="spk-en spk-current-prompt" dir="ltr">{prompt || (phase === 'starting' ? T.starting : '…')}</p>
        </div>
      ...
      {speaking && (
        <div className="spk-live-wave" aria-hidden="true">
          {[0, 1, 2, 3, 4].map((i) => <span key={i} className="spk-bar" />)}
        </div>
      )}
    </section>
  )
